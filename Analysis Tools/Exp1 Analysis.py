...
def mean_confidence_interval(data, confidence=0.95):
    ...
    return m, m+h, m-h
ri_list = ["01","02","03","04","05","06","07","08","09",10, \
           15,20,30,40,50,60,70,80,90,100,110,120]         
rm_list = [20,40,60]             
             
all_data_frame = pd.DataFrame([], columns=[\
                 "20_mean","20_sem","20_CI+","20_CI-","20_stdev","20_max","20_min",\
                 "40_mean","40_sem","40_CI+","40_CI-","40_stdev","40_max","40_min",\
                 "60_mean","60_sem","60_CI+","60_CI-","60_stdev","60_max","60_min"])                          

exp_name = "Exp1-2" 
export_folder = f"{exp_name}_data_analysis"                                        
all_data_export_path = str(Path(f"C:/Users/Cyrus/Documents/Emory/Lab/Dissertation Explorations/Dissertation Testing - Initial Inputs and Results/{exp_name} Results/{export_folder}/combined_data"))                                   

for current_rm in rm_list:
    for current_ri in ri_list:
        CI = 0.9 #confidence interval
        rounded = 3
        
        crit_name_element = "POP200_BKGD_RI"+ str(current_ri) + "_RM" + str(current_rm) + "r"
        print(crit_name_element)
        ...
        ad_a_stats_data = pd.DataFrame([[rounded_a,rounded_a_sem,rounded_a_CI_plus,rounded_a_CI_minus, \
                                    rounded_a_stdev,rounded_a_max,rounded_a_min]], index =[str(current_ri)+'_a'], \
                                    columns=[str(current_rm)+'_mean',str(current_rm)+'_sem',str(current_rm)+'_CI+',\
                                             str(current_rm)+'_CI-',str(current_rm)+'_stdev',\
                                             str(current_rm)+'_max',str(current_rm)+'_min'])
        all_data_frame = all_data_frame.append(ad_a_stats_data)
        ...
        ad_b_stats_data = pd.DataFrame([[rounded_b,rounded_b_sem,rounded_b_CI_plus,rounded_b_CI_minus, \
                                    rounded_b_stdev,rounded_b_max,rounded_b_min]], index =[str(current_ri)+'_b'], \
                                    columns=[str(current_rm)+'_mean',str(current_rm)+'_sem',str(current_rm)+'_CI+',\
                                             str(current_rm)+'_CI-',str(current_rm)+'_stdev',\
                                             str(current_rm)+'_max',str(current_rm)+'_min'])
        all_data_frame = all_data_frame.append(ad_b_stats_data)
        ...
        # current_rm
        # curent_ri
        ad_p_stats_data = pd.DataFrame([[rounded_p,rounded_p_sem,rounded_p_CI_plus,rounded_p_CI_minus, \
                                    rounded_p_stdev,rounded_p_max,rounded_p_min]], index =[str(current_ri)+'_PVAF'], \
                                    columns=[str(current_rm)+'_mean',str(current_rm)+'_sem',str(current_rm)+'_CI+',\
                                             str(current_rm)+'_CI-',str(current_rm)+'_stdev',\
                                             str(current_rm)+'_max',str(current_rm)+'_min'])
        all_data_frame = all_data_frame.append(ad_p_stats_data)
        ###############################################################################
...
writer = pd.ExcelWriter(all_data_export_path, engine='xlsxwriter')
all_data_frame.to_excel(writer, sheet_name='all_data')
writer.close()
# for input_file in file_list:
#     temp_data=None;
# filename = "foo.csv"
