...
rounded = 3
sum_ci = pd.DataFrame([], columns=['RI','RM','CI-10k','CI-20K','CI-40k','CI-60k','CI-80k'])
exp_name_list = ["Exp3I-t1","Exp3I-t2","Exp3I-t2-40k","Exp3I-t2-60k","Exp3I-t2-80k"]
...
for exp_name in exp_name_list:
    ...
    exp_element = "analysis_v3"
    # pre_crit_name_element_list = ['RI20_RM10_MMR100_']
    ...
    crit_name_element_list = ['RI20_RM10_','RI20_RM20_','RI20_RM30_','RI20_RM40_','RI20_RM50_', \
                              'RI40_RM10_','RI40_RM20_','RI40_RM30_','RI40_RM40_','RI40_RM50_', \
                              'RI60_RM10_','RI60_RM20_','RI60_RM30_','RI60_RM40_','RI60_RM50_', \
                              'RI80_RM10_','RI80_RM20_','RI80_RM30_','RI80_RM40_','RI80_RM50_', \
                              'RI100_RM10_','RI100_RM20_','RI100_RM30_','RI100_RM40_','RI100_RM50_', \
                              'RI120_RM10_','RI120_RM20_','RI120_RM30_','RI120_RM40_','RI120_RM50_', ] 
    #summary tasks:
    ...
    raw_data_folder = f"{folder_name}_data_analysis"
    ...
    summary_file_name = f'{exp_name}_combined_CI_extraction.xlsx'
    summary_export_path = str(Path(f"C:/Users/Cyrus/Documents/Emory/Lab/Dissertation Explorations/Dissertation Testing/{exp_name} Results/{export_folder}/{summary_file_name}"))
    
    for crit_name_element in crit_name_element_list:
        
        
        # export_file_name = f'{exp_name}_{crit_name_element}_CI_extraction.xlsx'
        ...
        # 
        file_list = glob.glob(f"{raw_data_path}/*{crit_name_element}*.xlsx")
        ...
        for input_file in file_list:
            ...
            # print(key_name)
            ri_start_index = input_file.rfind("_RI") + 3
            ri_end_index = input_file.rfind("_RM")
            ri_num = int(input_file[ri_start_index:ri_end_index])
            print(f'RI {ri_num}', end = ' ')
            
            rm_start_index = input_file.rfind("_RM") + 3
            rm_end_index = input_file.rfind("_MMR")
            rm_num = int(input_file[rm_start_index:rm_end_index])
            print(f'RM {rm_num}', end = ' ')
            
            tr_start_index = input_file.rfind("_TR") + 3
            tr_end_index = input_file.rfind("_EN")
            tr_str = input_file[tr_start_index:tr_end_index]
            tr_str_use = 'CI-' + tr_str
            # print(tr_str_use)
    
            try:
               temp_data = pd.pandas.read_excel(input_file, sheet_name='ordered')
            except FileNotFoundError:
                   print("data file not found!")  
             
            current_ci = temp_data.loc[temp_data["index"] == 0 , "CI"].values
            print(f' ci {current_ci}')
            ri_exist = False
            rm_exist = False
            new_index = len(sum_ci)
            # print(f' new index = {new_index}')
            # print(f'RI = {ri_num in sum_ci["RI"].values}')
            # print(f'RM = {rm_num in sum_ci.loc[sum_ci["RI"] ==  ri_num, "RM"].values}')
            if ri_num in sum_ci['RI'].values and rm_num in sum_ci.loc[sum_ci["RI"] ==  ri_num, "RM"].values:
                print('already Generated')
            else:
                print('generate')
                temp_set_up = pd.DataFrame([[ri_num,rm_num]], index = [new_index], columns=['RI','RM'])
                sum_ci = sum_ci.append(temp_set_up)
            
            
            sum_ci.reset_index(drop = True)
            # print(sum_ci.head())
            current_index = sum_ci.index[(sum_ci['RI']==ri_num) & (sum_ci['RM']==rm_num)].tolist()     
            # print(f'index = {current_index}')
            # print(f' tr = {tr_str_use}')
            print(f' index = {current_index},tr = {tr_str_use}')
            sum_ci.at[current_index,tr_str_use] = current_ci
            # print(sum_ci)
            # df.index[(df['points']==9) & (df['team']=='B')].tolist()
            # df.index[df['points']==7].tolist()
            
            # print(sum_ci.loc[sum_ci['RI'] == ri_num])
            # print(sum_ci.head())
            # if rm_num in sum_ci['RM']:
            #     rm_exist = True
            ...
            #         sum_ci.loc[sum_ci.index[i], unique_element_str] = CI_value[0]
            ...
            #     # sum_ci[unique_element_str] = analysis_data['CI']
...
if write_group_summary:
    ...
    sum_ci.to_excel(writer, sheet_name='CI')
