...
#Inputs########################################################################
#THIS CONTAINS ERROR(S)! look for "#ERROR"
schedule_list = [4]
        ...
        ###############################################################################
            ...
            if analysis_type == 'emitted_bx_histogram':
                if len(SE) == 0:
                    ...
                    #ERROR - Preparesubset does not care able schedule!!!!!
...
for current_schedule in schedule_list:
    for current_anchor in anchor_list:
        current_subset = rep_histogram_output[(rep_histogram_output['schedule'] == current_schedule) \
                                             &(rep_histogram_output['anchor'] == current_anchor)]
        # class_23 = titanic[(titanic["Pclass"] == 2) | (titanic["Pclass"] == 3)]
        for bin_x in range(1,histogram_bins+1):
            bin_name = 'bin_' + str(bin_x)
            current_bin = current_subset[bin_name]
            ...
            rounded_a_CI = round(a_mean - a_CI_plus,rounded)
            ...
            a_stats_data = pd.DataFrame([[current_schedule,current_anchor,(bin_x-1),rounded_a,rounded_a_CI,rounded_a_sem]], \
                                        columns=['schedule','anchor','bin','mean','ci','sem'])
...
for current_schedule in schedule_list:
    ...
    comparison_bins = stats_data[(stats_data['schedule'] == current_schedule) \
                                          &(stats_data['anchor'] == comparison_anchor)]['mean']
        
    for current_anchor in anchor_list:
        # if current_anchor == comparison_anchor:
        #     pass
        
        current_bins = stats_data[(stats_data['schedule'] == current_schedule) \
                                           &(stats_data['anchor'] == current_anchor)]['mean']
        ...
        temp_pc_df = pd.DataFrame([[current_schedule,comparison_anchor,current_anchor,current_correlation,current_pvalue]], \
                          columns=['schedule','compare_anchor','anchor','pc','p_val'])  
...
file_name_components = [crit_name_element,'pearsons']
